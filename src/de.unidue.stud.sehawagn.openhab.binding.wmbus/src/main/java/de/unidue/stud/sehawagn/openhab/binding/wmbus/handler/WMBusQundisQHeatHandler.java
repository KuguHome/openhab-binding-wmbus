...
public class WMBusQundisQHeatHandler extends WMBusDeviceHandler {
	...
	public void handleCommand(ChannelUID channelUID, Command command) {
		...
		if (command == RefreshType.REFRESH) {
			...
			// TODO manche VIB-DIB-Kombinationen wiederholen sich -> siehe JMbus-Bibliothek-Dokumentation betreffend DataRecord, wie das kodiert ist
			...
			if (wmbusDevice != null) {
				...
				case CHANNEL_CURRENTDATE: {
					...
					DataRecord record = wmbusDevice.findRecord(new byte[] { 0x04 }, new byte[] { 0x6d }); // TODO same as for Qcaloric 5,5 and Qwater 5,5
				...
				case CHANNEL_ERRORFLAGS: { // this is a current value / inst_val
					...
					DataRecord record = wmbusDevice.findRecord(new byte[] { 0x02 }, new byte[] { (byte) 0xFD, 0x17 });
					if (record != null) {
						newState = new DecimalType(record.getScaledDataValue());
					} else {
						logger.trace("handleCommand(): record not found in message");
