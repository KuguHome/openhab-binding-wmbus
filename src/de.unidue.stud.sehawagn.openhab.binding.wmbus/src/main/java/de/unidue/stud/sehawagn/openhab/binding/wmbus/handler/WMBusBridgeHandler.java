...
public class WMBusBridgeHandler extends ConfigStatusBridgeHandler {
    ...
    protected WMBusConnection wmbusConnection = null;
    ...
    public void initialize() {
        ...
        // check stick model
        if (!getConfig().containsKey(WMBusBindingConstants.CONFKEY_STARTUP_DELAY)
                || getConfig().get(WMBusBindingConstants.CONFKEY_STARTUP_DELAY) == null) {
            logger.error("Cannot open WMBus device. Startup delay not given.");
            updateStatus(ThingStatus.OFFLINE, ThingStatusDetail.OFFLINE.CONFIGURATION_ERROR,
                    "Cannot open WMBus device. Startup delay not given.");
            return;
        }
