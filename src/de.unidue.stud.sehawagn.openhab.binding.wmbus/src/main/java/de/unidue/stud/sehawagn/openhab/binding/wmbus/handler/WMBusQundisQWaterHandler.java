...
public class WMBusQundisQWaterHandler extends BaseThingHandler implements WMBusMessageListener {
    ...
    private final Logger logger = LoggerFactory.getLogger(WMBusQundisQWaterHandler.class);
    ...
    public WMBusQundisQWaterHandler(Thing thing) {
        super(thing);
        logger.debug("WMBusQundisQWaterHandler: new() for Thing" + thing.toString());
    }

    @Override
    public void initialize() {
        logger.debug("Initializing WMBusQundisQWaterHandler handler.");
    ...
    @Override
    public void dispose() {
        logger.debug("Disposing WMBusQundisQWaterHandler handler.");
    }

    // gets affected from onChangedWMBusDevice()
    @Override
    public void handleCommand(ChannelUID channelUID, Command command) {
        logger.trace("WMBusQundisQWaterHandler: handleCommand(): (1/5) command for channel " + channelUID.toString() + " command: " + command.toString());
        if (command == RefreshType.REFRESH) {
            logger.trace("WMBusQundisQWaterHandler: handleCommand(): (2/5) command.refreshtype == REFRESH");
            ...
            if (techemDevice != null) {
                logger.trace("WMBusQundisQWaterHandler: handleCommand(): (3/5) deviceMessage != null");
                /*
                ...
                switch (channelUID.getId()) {
                    case CHANNEL_RECEPTION: {
                        logger.trace("WMBusQundisQWaterHandler: handleCommand(): (4/5): got a valid channel: RECEPTION");
                    ...
                    case CHANNEL_ERRORDATE: {
                        logger.trace("WMBusQundisQWaterHandler: handleCommand(): (4/5): got a valid channel: ERRORDATE");
                        DataRecord record = findRecord(new byte[] { 0x32 }, new byte[] { 0x6c }); //TODO same as for Qundis Qcaloric 5,5
                    ...
                    case CHANNEL_CURRENTREADING: {
                        logger.trace("WMBusQundisQWaterHandler: handleCommand(): (4/5): got a valid channel: CURRENTREADING");
                        DataRecord record = findRecord(new byte[] { 0x0c }, new byte[] { 0x13 });
                        if (record != null) {
                            newState = new DecimalType(record.getScaledDataValue());
                        } else {
                            logger.trace("WMBusQundisQWaterHandler: handleCommand(): record not found in message");
                        }
                        break;
                    }
                    case CHANNEL_CURRENTVOLUMEFLOW: {
                        logger.trace("WMBusQundisQWaterHandler: handleCommand(): (4/5): got a valid channel: CURRENTVOLUMEFLOW");
                        //TODO why is type cast at 0xbb required? 0xbb already is a byte, right?
                        DataRecord record = findRecord(new byte[] { 0x02 }, new byte[] { (byte) 0xbb, 0x56 });
                        if (record != null) {
                            newState = new DecimalType(record.getScaledDataValue());
                        } else {
                            logger.trace("WMBusQundisQWaterHandler: handleCommand(): record not found in message");
                    ...
                    case CHANNEL_CURRENTDATE: {
                        logger.trace("WMBusQundisQWaterHandler: handleCommand(): (4/5): got a valid channel: CURRENTDATE");
                        DataRecord record = findRecord(new byte[] { 0x04 }, new byte[] { 0x6d }); //TODO same as for Qcaloric 5,5
                        if (record != null && record.getDataValueType() == DataValueType.DATE) {
                            Date date = (java.util.Date) record.getDataValue();
                            Calendar cal = Calendar.getInstance();
                            cal.setTime(date);
                    ...
                    case CHANNEL_PREVIOUSREADING: {
                        logger.trace("WMBusQundisQWaterHandler: handleCommand(): (4/5): got a valid channel: PREVIOUSREADING");
                        //TODO why is type cast at 0xCB required? 0xCB already is a byte, right?
                        DataRecord record = findRecord(new byte[] { (byte) 0xCC, 0x08 }, new byte[] { 0x13 });
                        if (record != null) {
                            newState = new DecimalType(record.getScaledDataValue());
                        } else {
                            logger.trace("WMBusQundisQWaterHandler: handleCommand(): record not found in message");
                    ...
                    case CHANNEL_PREVIOUSDATE: {
                        logger.trace("WMBusQundisQWaterHandler: handleCommand(): (4/5): got a valid channel: PREVIOUSDATE");
                        //TODO why is type cast at 0xC2 required? 0xC2 already is a byte, right?
                        DataRecord record = findRecord(new byte[] { (byte) 0xC2, 0x08 }, new byte[] { 0x6c }); //TODO same as for Qundis Qcaloric 5,5
                        if (record != null && record.getDataValueType() == DataValueType.DATE) {
                            Date date = (java.util.Date) record.getDataValue();
                            Calendar cal = Calendar.getInstance();
                            cal.setTime(date);
                    ...
                    case CHANNEL_LASTREADING: {
                        logger.trace("WMBusQundisQWaterHandler: handleCommand(): (4/5): got a valid channel: LASTREADING");
                        DataRecord record = findRecord(new byte[] { 0x4c }, new byte[] { 0x13 });
                        if (record != null) {
                            newState = new DecimalType(record.getScaledDataValue());
                        } else {
                            logger.trace("WMBusQundisQWaterHandler: handleCommand(): record not found in message");
                    ...
                    case CHANNEL_LASTDATE: {
                        logger.trace("WMBusQundisQWaterHandler: handleCommand(): (4/5): got a valid channel: LASTDATE");
                        DataRecord record = findRecord(new byte[] { 0x42 }, new byte[] { 0x6c }); //TODO same as for Qundis Qcaloric 5,5
                        if (record != null && record.getDataValueType() == DataValueType.DATE) {
                            Date date = (java.util.Date) record.getDataValue();
                            Calendar cal = Calendar.getInstance();
                            cal.setTime(date);
                    ...
                    default:
                        logger.debug("WMBusQundisQWaterHandler: handleCommand(): (4/5): no channel to put this value into found: " + channelUID.getId());
                        break;
                }
                logger.trace("WMBusQundisQWaterHandler: handleCommand(): (5/5) assigning new state to channel '" + channelUID.getId().toString() + "': " + newState.toString());
    ...
    private synchronized WMBusBridgeHandler getBridgeHandler() {
        logger.trace("WMBusQundisQWaterHandler: getBridgeHandler() begin");
        ...
        logger.trace("WMBusQundisQWaterHandler: getBridgeHandler() returning bridgehandler");
        return this.bridgeHandler;
    }

    private WMBusDevice getDevice() {
        logger.trace("WMBusQundisQWaterHandler: getDevice() begin");
        WMBusBridgeHandler bridgeHandler = getBridgeHandler();
        if (bridgeHandler == null) {
            logger.debug("thinghandler: getDevice() end: returning null");
            return null;
        }
        logger.trace("WMBusQundisQWaterHandler: getDevice() end: returning devicebyid");
    ...
    public void onNewWMBusDevice(WMBusDevice wmBusDevice) {
        logger.trace("WMBusQundisQWaterHandler: onNewWMBusDevice(): is it me?");
        if (wmBusDevice.getDeviceId().equals(deviceId)) {
            logger.trace("WMBusQundisQWaterHandler: onNewWMBusDevice(): yes it's me");
            logger.trace("WMBusQundisQWaterHandler: onNewWMBusDevice(): updating status to online");
            updateStatus(ThingStatus.ONLINE);
            logger.trace("WMBusQundisQWaterHandler: onNewWMBusDevice(): calling onChangedWMBusDevice()");
            onChangedWMBusDevice(wmBusDevice);
        }
        logger.trace("WMBusQundisQWaterHandler: onNewWMBusDevice(): no");
    }

    @Override
    public void onChangedWMBusDevice(WMBusDevice wmBusDevice) {
        logger.trace("WMBusQundisQWaterHandler: onChangedWMBusDevice(): is it me?");
        if (wmBusDevice.getDeviceId().equals(deviceId)) {
            techemDevice = wmBusDevice;
            logger.trace("WMBusQundisQWaterHandler: onChangedWMBusDevice(): yes");
            ...
            if (record != null) {
                logger.trace("WMBusQundisQWaterHandler: onChangedWMBusDevice(): this is a message with non-OMS manufacturer-specific value, ignoring this message");
            } else {
                // refresh all channels -> handleCommand()
                logger.trace("WMBusQundisQWaterHandler: onChangedWMBusDevice(): inform all channels to refresh");
                for (Channel curChan : getThing().getChannels()) {
                    handleCommand(curChan.getUID(), RefreshType.REFRESH);
                }
            }
        } else {
            logger.trace("WMBusQundisQWaterHandler: onChangedWMBusDevice(): no");
        }
        logger.trace("WMBusQundisQWaterHandler: onChangedWMBusDevice(): return");
    }
}
