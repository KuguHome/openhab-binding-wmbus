...
/**
 * The {@link LandisGyrHandlerFactory} covers logic specific to Landis+Gyr devices.
 *
 * @author Łukasz Dywicki - Initial contribution
 */
@Component(service = { LandisGyrHandlerFactory.class, BaseThingHandlerFactory.class, ThingHandlerFactory.class })
public class LandisGyrHandlerFactory extends BaseThingHandlerFactory {

    // OpenHAB logger
    private final Logger logger = LoggerFactory.getLogger(LandisGyrHandlerFactory.class);

    public LandisGyrHandlerFactory() {
        logger.debug("Landis+Gyr handler factory starting up.");
    }

    @Override
    public boolean supportsThingType(ThingTypeUID thingTypeUID) {
        return LandisGyrBindingConstants.SUPPORTED_THING_TYPES.contains(thingTypeUID);
    }
    ...
    protected ThingHandler createHandler(Thing thing) {
        ...
        if (thingTypeUID.equals(LandisGyrBindingConstants.THING_TYPE_HEAT_METER)) {
            ...
            return new LandisGyrThingHandler(thing);
        }
