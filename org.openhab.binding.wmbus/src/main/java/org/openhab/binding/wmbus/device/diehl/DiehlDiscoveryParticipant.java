...
/**
 * Discovers Diehl devices.
 *
 * @author Łukasz Dywicki - Initial contribution.
 */
@Component(immediate = true)
public class DiehlDiscoveryParticipant extends AbstractWMBusDiscoveryParticipant implements WMBusDiscoveryParticipant {

    private final Logger logger = LoggerFactory.getLogger(DiehlDiscoveryParticipant.class);

    @Override
    public @NonNull Set<ThingTypeUID> getSupportedThingTypeUIDs() {
        return DiehlBindingConstants.SUPPORTED_THING_TYPES;
    }
    ...
    public @Nullable ThingUID getThingUID(WMBusDevice device) {
        ...
        if (!DiehlBindingConstants.MANUFACTURER_IDS.contains(message.getSecondaryAddress().getManufacturerId())) {
            return null;
        }

        if (!DiehlBindingConstants.SUPPORTED_DEVICE_VARIANTS.containsKey(device.getDeviceType())) {
            logger.trace("Found unsupported Diehl device {}, ommiting it from discovery results.",
                    device.getDeviceType());
            return null;
        }

        try {
            WMBusDevice qloudDevice = new DiehlWMBusDevice(message, device.getAdapter());
    ...
    public DiscoveryResult createResult(WMBusDevice device) {
        ...
        if (thingUID != null) {
            ...
            properties.put(Thing.PROPERTY_VENDOR, "Diehl GmbH");
            ...
            return DiscoveryResultBuilder.create(thingUID).withProperties(properties)
                    ...
                    .withThingType(DiehlBindingConstants.SUPPORTED_DEVICE_VARIANTS.get(device.getDeviceType()))
    ...
    protected ThingTypeUID getThingType(WMBusDevice device) {
        switch (device.getOriginalMessage().getSecondaryAddress().getDeviceType()) {
            case HEAT_COST_ALLOCATOR:
                return DiehlBindingConstants.THING_TYPE_HEAT_COST_ALLOCATOR;
        }
