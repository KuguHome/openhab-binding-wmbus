...
/**
 * Discovers eie devices and adds additional layer for decoding on top of generic WM-Bus devices.
 *
 * @author Łukasz Dywicki - initial contribution.
 */
@Component
public class EieDiscoveryParticipant implements WMBusDiscoveryParticipant {

    private final Logger logger = LoggerFactory.getLogger(EieDiscoveryParticipant.class);

    private BindingConfiguration configuration;

    @Override
    public @NonNull Set<ThingTypeUID> getSupportedThingTypeUIDs() {
        return EieBindingConstants.SUPPORTED_THING_TYPES;
    }

    @Override
    public @Nullable ThingUID getThingUID(WMBusDevice device) {
        ThingTypeUID type = EieBindingConstants.THING_TYPE_EIE_SMOKE_DETECTOR;
    ...
    public DiscoveryResult createResult(WMBusDevice device) {
        if (isEieSmokeDetector(device)) {
            String label = "Eie smoke detector #" + device.getDeviceId();

            Map<String, Object> properties = new HashMap<>();
            properties.put(WMBusBindingConstants.PROPERTY_DEVICE_ADDRESS, device.getDeviceAddress());
            properties.put(Thing.PROPERTY_VENDOR, "Eie");
            ...
            return DiscoveryResultBuilder.create(thingUID).withProperties(properties)
                    ...
                    .withThingType(EieBindingConstants.THING_TYPE_EIE_SMOKE_DETECTOR)
    ...
    private boolean isEieSmokeDetector(WMBusDevice device) {
        WMBusMessage message = device.getOriginalMessage();

        if (!EieBindingConstants.EIE_MANUFACTURER_ID.equals(message.getSecondaryAddress().getManufacturerId()) ||
