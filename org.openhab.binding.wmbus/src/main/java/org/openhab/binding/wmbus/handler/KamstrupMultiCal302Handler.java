...
/**
 * The {@link KamstrupMultiCal302Handler} class defines Device/thing handler for the Kamstrup MultiCal 302 heat meter
 * (Wärmezähler)
 *
...
public class KamstrupMultiCal302Handler extends WMBusDeviceHandler {
    ...
    /*
     * DIB:03, VIB:06 -> descr:ENERGY, function:INST_VAL, scaled value:0.0, unit:WATT_HOUR, Wh -- current energy total
     * reading in Wh
     * DIB:43, VIB:06 -> descr:ENERGY, function:INST_VAL, storage:1, scaled value:0.0, unit:WATT_HOUR, Wh -- previous
     * energy total reading in Wh
     * DIB:03, VIB:14 -> descr:VOLUME, function:INST_VAL, scaled value:0.0, unit:CUBIC_METRE, m³ -- current water volume
     * total reading in m³
     * DIB:42, VIB:6C -> descr:DATE, function:INST_VAL, storage:1, value:Thu Nov 30 00:00:00 CET 2017 -- previous
     * reading timestamp
     * DIB:02, VIB:2D -> descr:POWER, function:INST_VAL, scaled value:0.0, unit:WATT, W -- current water heat power
     * reading in W
            ...
            if (wmbusDevice != null) {
                ...
                switch (channelUID.getId()) {
                    ...
                    case CHANNEL_CURRENTENERGYTOTAL: {
                        ...
                        if (record != null) {
                            newState = new DecimalType(record.getScaledDataValue() / 1000); // Wh to kWh (usual value
                                                                                            // unit)
                        } else {
                            logger.trace("handleCommand(): record not found in message");
                        }
